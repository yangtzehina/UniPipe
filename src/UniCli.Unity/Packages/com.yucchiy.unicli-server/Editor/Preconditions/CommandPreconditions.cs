...
{
    /// <summary>
    /// Reads the editor state a precondition check needs. Exists so the check itself can be
    /// unit tested — <see cref="EditorApplication"/>'s statics cannot be put into a chosen
    /// state from a test.
    /// </summary>
    ...
    /// <summary>
    /// What a command declared about itself, resolved once when the handler is registered.
    /// </summary>
    ...
    ///
    /// <see cref="DirtyScenePolicy"/> anticipated this: "If a third kind of pre-execution check
    /// appears, consolidate them into a precondition pipeline on CommandDispatcher rather than
    /// merging the checks into one type." This is that pipeline.
    ///
    /// What lives here is everything decidable from the command's identity alone. Checks that
    /// need the deserialized request — the dirty-scene policy needs a <c>dirtyAction</c> field
    /// and, for scene commands, which scenes a given request touches — stay with the handler,
    /// because deserialization happens below this layer. Commands still declare
    /// <see cref="CommandPreconditionAttribute.ReplacesOpenScenes"/> so the risk is visible in
    /// metadata even where enforcement cannot be lifted.
    /// </summary>
    public static class CommandPreconditions
    {
        ...
        public static CommandPrecondition Resolve(Type handlerType)
        {
            ...
            var resolved = attribute == null
                ...
                : new CommandPrecondition(attribute.EditorState, attribute.ReplacesOpenScenes, attribute.Destructive);
